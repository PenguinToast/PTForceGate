...
function tryConnect(direction, ignoreId)
  ...
  local objects = world.objectQuery(searchArea[1], searchArea[2], {withoutEntityId = entity.id(), name = "ptforcegate"})
  ...
  if #objects > 0 then
    ...
    if objects[i] ~= nil then
      ...
      if not collisionBetweenPoints(pos, world.entityPosition(targetId)) then
...
function checkLoS()
  ...
  for direction,data in pairs(storage.connectedGates) do
    ...
    if collisionBetweenPoints(pos, world.entityPosition(data.gateId)) then
...
function collisionBetweenPoints(source, target) 
